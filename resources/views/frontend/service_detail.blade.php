    ...
    <section class="single-details-section">
        <div class="container">
            <div class="row mt-5">
                ...
                <div class="col-md-6">
                    <h3 class="sngl-hdr wow fadeInRight" data-wow-delay="0.5s">{{$service->name}}</h3>
                    @php
                        $prices = json_decode($service->price);
                    @endphp
                    <div class="price-otr mt-3 wow fadeInRight" data-wow-delay="0.6s">Price start at  <i class="far fa-rupee-sign"></i>{{$prices[0]->price}}</div>
                    ...
                    <div class="ettra-info my-3 wow fadeInRight" data-wow-delay="0.8s">
                        <p>{{$service->long_description}}</p>
                    </div>
                    <div class="action-otr my-2 wow fadeInRight"  data-wow-delay="1s">
                        @auth
                            @foreach ($prices as $price)
                            <form action="{{route('pay', ['for' => 'service', 'type' => 'booking', 'id' => $service->id ])}}" method="post">
                                @csrf
                                <input type="hidden" name="amount" value="{{$price->price}}">
                                <button type="submit" class="btn btn-primary">Book <i class="far fa-rupee-sign"></i>{{$price->price}}</button>
                            </form>
                            @endforeach
                        @else
